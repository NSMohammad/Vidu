...
//        }
    ...
    func createLocalOffer(mediaConstraints: RTCMediaConstraints) {
        ...
        localPeer?.offer(for: mediaConstraints, completionHandler: { (sessionDescription, error) in
            ...
            print(self.webSocketListener!.id)
...
//        print("remoteper ice server\(webSocketListener?.iceServers)")
//        if let webSocketListener = webSocketListener {
        config.iceServers = webSocketListener!.iceServers
//        }
//        config.iceServers = [RTCIceServer(urlStrings: ["stun:stun.l.google.com:19302"])]
//        print("config ice server \(config.iceServers)")
//        print("done iceserver")
        remoteParticipant.peerConnection = (peerConnectionFactory?.peerConnection(with: config, constraints: sdpConstraints, delegate: nil))!
    }
}
    ...
    func peerConnection(_ peerConnection: RTCPeerConnection, didGenerate candidate: RTCIceCandidate) {
        if peerConnection == self.localPeer {
            ...
            if self.webSocketListener!.userId != nil {
                ...
                print("icelocal endpoint- \(iceCandidateParams)")
...
//            con_YeNMf1F79c_str_CAM_NJev_con_KJt2hqsv74
            
            print("iceremote endpoint- \(iceCandidateParams)")
            self.webSocketListener!.sendJson(method: "onIceCandidate", params: iceCandidateParams)
